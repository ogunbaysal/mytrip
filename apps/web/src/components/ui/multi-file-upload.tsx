...
import { Upload, X, Loader2, Plus, ImageIcon, AlertCircle } from "lucide-react";
  ...
  const handleFiles = async (files: FileList | File[]) => {
    ...
    for (const file of fileArray) {
      ...
      if (file.size > maxSize) {
        setError(`Dosya boyutu ${(maxSize / 1024 / 1024).toFixed(0)}MB'dan küçük olmalıdır`);
        return;
      }
    ...
    try {
      ...
      let result: { urls: string[]; errors?: string[] };
      if (uploadFn) {
        result = await uploadFn(validFiles);
      } else {
        const { api } = await import("@/lib/api");
        result = await api.owner.upload.multiple(validFiles);
      }
