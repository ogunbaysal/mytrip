...
export const adminRolePermissions = pgTable("admin_role_permissions", {
  ...
  permissionId: text("permission_id").notNull().references(() => adminPermissions.id, { onDelete: "cascade" }),
}, (t) => ({
...
export const adminSession = pgTable("admin_session", {
  ...
  userId: text("user_id").notNull().references(() => admin.id, { onDelete: "cascade" }),
});
